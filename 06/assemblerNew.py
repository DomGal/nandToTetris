...
class Command:
    ...
    def __parseACommand(self):
            ...
            #raise ValueError(f"{self.line} is not an A command")
        return self.line[1:]
...
class Preprocessor:
    ...
    def __getDefaultSymbolsDict(self):
        ...
        return symbolsDict
    """
    def __reconstructSymbolsDict():
        # finish implementation
        # from second subtract 1
        # from next one repeat
    """
    def process(self):
        ...
        for line in self.fileList:
            ...
            if currentCommand.getCommandType() == CommandType.A_COMMAND:
                ...
                if value not in self.symbolsDict.keys() and currentCommand.isSymbolic():
                    ...
                    #debugging
                    print(value, self.nextRef)
                    self.nextRef += 1
            
            self.fileListClean.append(cLine)
            self.lineNumber += 1

            #self.__reconstructSymbolsDict()
            #debuggin
...
if __name__ == "__main__":
    ...
    print("Hack Assembler finished.\nDuration: {0:2f} ms.".format(timeDelta))